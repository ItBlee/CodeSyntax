...
import Server.api.Compiler;
import Server.api.Formatter;
import Server.api.ImageConverter;
...
public class ServerListener extends Thread implements Runnable{
            ...
            while(true) {
                try {
                    ...
                    if (line.equalsIgnoreCase("renewed")) {
                        ...
                        recheckIfTargetAtManager(user);//For server managerFrame (bỏ qua)
                        continue;
                    }
                    ...
                    ClientPacket packet = requestHandle(line, user.getSecretKey());
    ...
    private void verify() throws IOException, NullPointerException {
        ...
        //For server managerFrame (bỏ qua)
    ...
    public static void recheckIfTargetAtManager(User user) {
        String managerCurrentTarget = Server.managerFrame.textField.getText();
        ...
        if (managerCurrentTarget.equals(user.getUID())
            ...
            Server.managerFrame._btnCheck.doClick();
        }
    }
     ...
     * @return ClientPacket - Gói dữ liệu Client
     */
    public ClientPacket requestHandle(String data, String secretKey) {
        String decryptJson = Encryptor.decrypt(data, secretKey);
        user.addRequestList(JsonParser.parseString(decryptJson).toString());
        user.addDateList(LocalDateTime.now().toString());
        return ClientPacket.unpack(decryptJson); //giả mã bằng secret key
    }
     ...
     */
    public String responseHandle(ClientPacket dataPacket, String secretKey) throws IOException, NullPointerException, InterruptedException {
            ...
            case "FORMAT":
                formatExecutors.execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            String data = Encryptor.encrypt(serverPacket.pack(), secretKey);
                            send(data);
        ...
        ServerPacket serverPacket = new ServerPacket(description, format, output, statusCode, memory, cpuTime);
        user.addResponseList(JsonParser.parseString(serverPacket.pack()).toString());
        return Encryptor.encrypt(serverPacket.pack(), secretKey); //mã hóa bằng secret key trước khi gửi
    }
}
